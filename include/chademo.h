...
private:
  ...
  static void SetTargetBatteryVoltage(uint16_t vtg) {
    targetBatteryVoltage = vtg;
  }
  static void SetChargeCurrent(uint8_t cur);
  static void SetEnabled(bool enabled);

  static void SetParkPosition(bool pos) {
    parkingPosition = !pos;
  ...
  static void
  CheckSensorDeviation(uint16_t internalVoltage); // Must be called every 100 ms
};
