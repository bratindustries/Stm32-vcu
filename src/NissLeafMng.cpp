...
void NissLeafMng::Task10Ms(int16_t final_torque_request) {
 ...
 if (quickChargeState == QC_START) {
      ...
      // Hold relay-enable phase before allowing a non-zero request.
      bytes[0] = 0x00;
      bytes[1] = 0x64;
      bytes[2] = 0xC4;
      bytes[3] = 0xA0;
      ...
      // Normal AC-charge command from current master.
      bytes[0] = 0x30;
      bytes[1] = OBCpwr;
      bytes[2] = 0x20;
      bytes[3] = 0xAC;
      ...
      bytes[5] = 0x3C;
      bytes[6] = mprun10;
      bytes[7] = 0x92; // DCDC setpoint in PDM
    }
    ...
    if (BMSspoof) {
      ...
      // 0x03 Charge and normal stop req. Other values call for a caution lamp
      // which we don't need bits 11-12 LB relay cut req 0x00 no req
      ...
      bytes[1] = (Ibatt & 0x07) << 5; // LSB current bits 7-5. Dont need to mess
                                      // with bits 0-4 for now as 0 works.
      bytes[2] = Vbatt >> 2;
      bytes[3] =
    ...
    if (BMSspoof) {
      ...
      // Max charging power in bits 13-20. 10 bit unsigned scale 0.25.Byte 1
      // limit in kw.
...
void NissLeafMng::Task100Ms() {
  ...
  if (Param::GetInt(Param::BMS_Mode) ==
      4) // Check ran here as this runs on Zombie power on.
  {
    BMSspoof = false;
  } else {
    BMSspoof = true;
  ...
  if (SendCan == true &&
      opmode ==
          MOD_OFF) // stop sending CAN if we are sending CAN and enter OFF mode
  {
    if (SleepCount == 0) {
      SendCan = false;
  ...
  if (SendCan == true) // only send CAN when needed
  {
    ...
    // FIXME: This is a very sloppy implementation. Thanks. I try:) Dala: This
    // message is not 10ms, it is 100ms
    bytes[0] = 0x00;
    bytes[1] = 0x00;
    bytes[2] = 0x06;
    bytes[3] = 0xc0;
